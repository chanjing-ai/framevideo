...
**Both are first-class options.** Shaders are provided by the `@framevideo/shader-transitions` package — import from the package instead of writing raw GLSL. CSS transitions are simpler to set up. Choose based on the effect you want, not based on which is easier.
