---
name: tailwind
description: Tailwind CSS v4.2 browser-runtime patterns for FrameVideo compositions. Use when scaffolding or editing projects created with `framevideo init --tailwind`, writing Tailwind utility classes in composition HTML, adding CSS-first Tailwind v4 theme tokens, debugging v3 vs v4 syntax, or deciding when to compile Tailwind to CSS instead of using the browser runtime.
---
...
- The user asks for Tailwind in a FrameVideo composition.
- A project was created with `framevideo init --tailwind`.
...
- FrameVideo waits for `window.__tailwindReady` before frame capture starts.
...
1. Confirm the project was scaffolded with `framevideo init --tailwind`.
...
7. Run `npx framevideo validate` and render a short proof.
