---
name: typegpu
description: TypeGPU and raw WebGPU adapter patterns for FrameVideo. Use when creating GPU-rendered compositions with TypeGPU, raw WebGPU, WGSL fragment shaders, compute pipelines, liquid glass effects, particle systems, or any canvas layer driven by navigator.gpu that responds to FrameVideo fv-seek events.
---
...
- Initialize WebGPU asynchronously (`await navigator.gpu.requestAdapter()`), but register all GSAP tweens **synchronously** — before any `await`. The FrameVideo player reads the timeline immediately at page load.
- Render from FrameVideo time, not `performance.now()`.
- Listen for the `fv-seek` event and re-render at exactly that time.
...
<script>
  (async () => {
    ...
    render(0);
    window.addEventListener("fv-seek", (e) => render(e.detail.time));
  })();
</script>
```
...
- No `requestAnimationFrame` for the render loop — render only in response to `fv-seek`.
- No `performance.now()` for animation time — read `window.__fvTypegpuTime` or `e.detail.time`.
