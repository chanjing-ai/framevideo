---
name: gsap
description: GSAP animation reference for FrameVideo. Covers gsap.to(), from(), fromTo(), easing, stagger, defaults, timelines (gsap.timeline(), position parameter, labels, nesting, playback), and performance (transforms, will-change, quickTo). Use when writing GSAP animations in FrameVideo compositions.
---
...
- **repeat** — finite number; never `-1` in FrameVideo. Compute repeats from the visible duration. **yoyo** — alternates direction with repeat.
...
- FrameVideo adapter source: `packages/core/src/runtime/adapters/gsap.ts`.
