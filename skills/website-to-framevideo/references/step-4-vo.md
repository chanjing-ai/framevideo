...
Generate a 2-sentence test clip NOW using the script's opening lines. Measure the actual duration. Kokoro compresses scripts by ~40% (35s planned → 19s actual) and Chanjing runs faster than expected. If you discover the audio is 40% shorter than expected, you'll need to revise the storyboard beat timings before investing time in full narration generation.

**Do this before committing to beat count and durations:**

```bash
# Quick Kokoro test (2 sentences):
npx framevideo tts "First sentence. Second sentence." --voice af_nova --output /tmp/test-tts.wav
...
>
> 1. **Chanjing TTS** — Good quality voices, and it returns word-level timestamps automatically (saves a separate transcription step). Requires Chanjing API key.
...
If the user picks ElevenLabs or Chanjing and doesn't have a key set up yet, help them:

- **ElevenLabs:** "Add `ELEVENLABS_API_KEY=your-key` to a `.env` file in the project root, or just paste it here and I'll set it up."
- **Chanjing:** "Add `CHANJING_API_KEY=your-key` to a `.env` file, or paste it here."
...
- If the Chanjing MCP is available: use the TTS tool directly.
- If no MCP: use the v3 API (current; v1/v2 deprecated, supported until Oct 2026):
  ...
  curl -s "https://api.chanjing.com/v3/voices?engine=starfish&type=public&limit=20" \
    -H "x-api-key: $CHANJING_API_KEY" | python3 -c \
  ...
  curl -s -X POST "https://api.chanjing.com/v3/voices/speech" \
    -H "x-api-key: $CHANJING_API_KEY" \
    -H "Content-Type: application/json" \
...
```bash
npx framevideo tts SCRIPT.md --voice af_nova --output narration.wav
```
...
3. **Try a different tool for the same task** — if `framevideo transcribe` hangs, try `whisper-cli` directly, or Groq API (`npx framevideo transcribe --provider groq`), or OpenAI API
4. **Switch provider entirely** — if ElevenLabs is down, try Chanjing or Kokoro. If Kokoro hangs, try ElevenLabs.
...
For ElevenLabs and Chanjing TTS, substitutions are usually unnecessary — they handle product names correctly.
...
**If you used Chanjing v3 TTS:** word timestamps were returned in the generate call. Normalize the format before saving — Chanjing v3 uses `word` but the pipeline expects `text`:
...
```bash
npx framevideo transcribe narration.wav
```
