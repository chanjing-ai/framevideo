...
| `clockWipe()`                      | use HF's `sdf-iris` shader-transition (`npx framevideo add sdf-iris`)                    |
...
(iris, ripple, zoom, glitch), use HF's [shader-transitions](https://framevideo.chanjing.com/catalog/blocks)
package. They produce richer output than manual GSAP transforms.

```bash
npx framevideo add sdf-iris
```

Then in the composition:

```html
<div id="iris-transition" class="fv-shader-transition" data-start="1.5" data-duration="0.5">
