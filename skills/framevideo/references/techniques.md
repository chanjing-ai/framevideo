...
13 primitive animation techniques from production FrameVideo videos — SVG drawing, kinetic typography, variable fonts, WebGL shaders, motion-path, etc. Compose these into beats; they are the building blocks, not finished recipes. Each entry includes a minimal code pattern you can adapt.
...
The FrameVideo runtime handles video seeking and playback.
...
**Never do:** equalizer bars, spectrum analyzers, waveform displays, strobing, rainbow color cycling. The audio provides _timing and intensity_; the visual vocabulary still comes from the brand. See `skills/framevideo/references/audio-reactive.md` for the full API and anti-patterns.
