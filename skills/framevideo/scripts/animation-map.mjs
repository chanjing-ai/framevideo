#!/usr/bin/env node
// animation-map.mjs — FrameVideo animation map for agents
//
...
const {
  ...
  await importPackagesOrBootstrap(["@framevideo/producer"], {
    npmPackages: [framevideoPackageSpec("@framevideo/producer")],
  })
)["@framevideo/producer"];
...
const OUT_DIR = resolve(args.out ?? ".framevideo/anim-map");
...
async function seekTo(session, t) {
  await session.page.evaluate((time) => {
    if (window.__fv && typeof window.__fv.seek === "function") {
      window.__fv.seek(time);
      return;
    }
