# FrameVideo — Complete Capabilities Inventory

Everything possible in FrameVideo as of today's workspace, synthesized from direct source reads of all 7 packages, 16 skills, and the full registry.
...
- **Linter:** 60+ rules. Run `npx framevideo lint` before render. Catches missing timelines, overlapping clips, broken paths, GSAP errors.
...
| 2   | **Animation engines (6 adapters)**                   | GSAP + 15 plugins, Anime.js v4, CSS @keyframes, WAAPI, Lottie (lottie-web + dotlottie), Three.js (fv-seek event)                                                                                                                |
...
| 9   | **Three.js / WebGL custom scenes**                   | Full 3D: AnimationMixer, custom GLSL, post-processing, GLTF models, lights, cameras, materials — all deterministic via fv-seek                                                                                                  |
...
| 15  | **Player web component**                             | `<framevideo-player>` with seek/play/pause API, 11 events, media mirror, runtime auto-inject                                                                                                                                   |
...
| 22  | **Skills (16)**                                      | framevideo, cli, media, registry, contrast, animation-map, website-to-framevideo, remotion, gsap, animejs, css-animations, waapi, lottie, three, tailwind, contribute-catalog                                                 |
...
- Looping handled by `<framevideo-player>`, not GSAP `repeat: -1`
...
| ---------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| GSAP (createGsapAdapter)           | The primary timeline + all tweens registered on `window.__timelines[<id>]`    | CDN `https://cdn.jsdelivr.net/npm/gsap@3.14.2/dist/gsap.min.js` | Plugins via standard GSAP register; FrameVideo does NOT patch THREE.Clock (uses `__fvThreeTime` instead) |
| Anime.js v4 (createAnimeJsAdapter) | Anime instances pushed to `window.__fvAnime`                                  | CDN `animejs@4.0.2/lib/anime.iife.min.js` or ESM                | Adapter multiplies composition seconds by 1000 for ms                                                     |
...
| Lottie (createLottieAdapter)       | `window.__fvLottie` array; supports lottie-web + dotlottie-web                | CDN `lottie.min.js` + `@lottiefiles/dotlottie-web`              | `goToAndStop(time*1000)` or `setCurrentRawFrameValue` / `seek(%)`                                         |
| Three.js (createThreeAdapter)      | `window.__fvThreeTime` + dispatches `CustomEvent("fv-seek", {detail:{time}})` | ESM CDN `three@0.181.2/+esm`                                    | Composition's render loop listens to `fv-seek`; pattern: `mixer.setTime(time)`                            |

### GSAP plugins (documented patterns)

- **TextPlugin** — text mutation in `tl.call` (skills/gsap/references/effects.md)
- **MotionPathPlugin** — curve-constrained tweens (skills/framevideo/references/techniques.md)
- **CustomEase** — bezier eases imported from Remotion-style timing
- **ScrollTrigger / Flip / SplitText / Draggable / Inertia / Observer / ScrambleText / CustomWiggle / CustomBounce / ScrollSmoother / GSDevTools** — work natively if loaded and tweens are on the registered paused timeline, but no special FrameVideo adapter
- Producer injects ScrollTrigger CDN automatically when needed (packages/producer/src/services/htmlCompiler.ts)

---

## 3. Shader transitions — @framevideo/shader-transitions
...
- Tuning: `?__fv_shader_capture_scale=` (0.25–1), `?__fv_shader_loading=` (internal|player|none)
...
- Engine mode skips GL/capture when `window.__FV_VIRTUAL_TIME__` set (producer uses metadata)
...
Documented in skills/framevideo/references/transitions/ across 14 category files. All GSAP-driven, none mixable with shader transitions in same composition.
...
| ------------------------------------------ | -------- | ----------------- |
| framevideo transcribe (local whisper.cpp) | JSON     | Word-level        |
...
| framevideo tts → transcribe chain         | wav→json | Word-level        |

### Positioning helpers

- Landscape: bottom 80–120px centered
- Portrait: ~600–700px from bottom
- `window.__framevideo.fitTextFontSize(text, {maxWidth, fontFamily, fontWeight})` for dynamic sizing
...
Documented in skills/framevideo/references/html-in-canvas-patterns.md (504 lines).
...
```js
window.addEventListener("fv-seek", (e) => {
...
(From skills/framevideo/references/techniques.md)
...
- FrameVideo extracts frames at render via videoFrameInjector (avoids unreliable headless `<video>` playback)
...
Install: `npx framevideo add <name>` for blocks/components, `framevideo init <dir> --example <name>` for examples.
...
| publish           | Zip + upload + return framevideo.dev URL                                                                                                                                                                                                                                                                                |
...
| skills            | Run `npx skills add chanjing-ai/framevideo --all`                                                                                                                                                                                                                                                                        |
...
### Website capture (`framevideo capture <url>`)

Detects these libraries on captured sites (for context labeling): GSAP / ScrollTrigger, Three.js, Lottie, Anime.js, PixiJS, Babylon.js, Rive, Matter.js, Lenis, Framer Motion, Tailwind CSS, WebGL (shader fingerprinting). Captured outputs feed the website-to-framevideo skill workflow.
...
## 15. Player — `<framevideo-player>` web component
...
Loads `RUNTIME_CDN_URL` (`@framevideo/core/dist/framevideo.runtime.iife.js`) if missing `__fv`/`__player` but timelines exist.
...
- File server injects `FV_EARLY_STUB`, `FV_BRIDGE_SCRIPT`, virtual-time so `window.__fv` bridges `window.__player.renderSeek`
- HDR-aware shader transition compositing via `window.__fv.transitions` metadata
...
Access via `window.__framevideo.getVariables()`. Override at render time:

```bash
npx framevideo render --variables '{"brand":"Linear","primary":"#5E6AD2"}'
npx framevideo render --variables-file vars.json
npx framevideo render --strict-variables  # error if unused / mismatched
```
...
- Reads `data-variable-values` merged with own defaults into `window.__fvVariablesByComp[<id>]`
...
| Global                                | Purpose                                                                 |
| ------------------------------------- | ----------------------------------------------------------------------- |
| `__framevideo`                       | `{ fitTextFontSize, getVariables }`                                     |
...
| `__FV_PARITY_MODE`                    | Engine parity flag                                                      |
| `__FV_FPS`                            | Render FPS hint                                                         |
| `__FV_MAX_DURATION_SEC`               | Engine clamp                                                            |
| `__FV_VIRTUAL_TIME__`                 | Set by engine in render mode (shader-transitions switches paths)        |
| `__fvThreeTime`                       | Current time for Three.js adapter                                       |
| `__FV_PICKER_API`                     | Element picker hook for Studio                                          |
| `__fvAnime`                           | Array of Anime.js instances                                             |
| `__fvLottie`                          | Array of Lottie animations                                              |
| `__fvVariables`                       | Resolved root variables                                                 |
| `__fvVariablesByComp`                 | Per-composition variable map                                            |
| `__fvRuntimeTeardown`                 | Cleanup function                                                        |
| `__tailwindReady`                     | Tailwind browser build ready (gates capture)                            |
| `__fv.transitions`                    | Shader transition metadata (read by producer for HDR-aware compositing) |
...
| Skill                     | Purpose                                                                 |
| ------------------------- | ----------------------------------------------------------------------- |
| framevideo               | Core framework skill (composition authoring)                            |
| framevideo-cli           | All CLI commands as an agent skill                                      |
| framevideo-media         | Media workflows (TTS, transcribe, captions integration)                 |
| framevideo-registry      | Installing blocks/components                                            |
| framevideo-contrast      | WCAG audit (scripts/contrast-report.mjs)                                |
| framevideo-animation-map | Per-tween bbox + flags report                                           |
| website-to-framevideo    | Capture → DESIGN.md → brief → storyboard+script → VO → build → validate |
| remotion-to-framevideo   | Migration patterns + API map + CustomEase                               |
...
## 23. References inventory (skills/framevideo/references/)
...
**Guides:** 4k-rendering, claude-design, common-mistakes, deploy, gsap-animation, hdr, html-in-canvas, framevideo-vs-remotion, open-design, performance, prompting, remove-background, rendering, timeline-editing, troubleshooting, video-editor-cheatsheet, website-to-video, etc.
