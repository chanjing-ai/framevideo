...
Load the `framevideo` skill — it has the rules for data attributes, timeline contracts, deterministic rendering, and layout. Read it now if you haven't already this session.
...
```bash
cp -r skills/website-to-framevideo/assets/sfx/ <project-dir>/sfx/
# If skill is installed elsewhere:
find . -path "*/website-to-framevideo/assets/sfx" -exec cp -r {} <project-dir>/sfx/ \;
```
...
the skill lives outside it — run this to find it:
  ...
  find "$HOME" -path '*/website-to-framevideo/references/beat-builder-guide.md' -maxdepth 10 2>/dev/null | head -1
...
and file references. Follow its workflow exactly:
  build → lint (`npx framevideo lint .`)
        → snapshot (`npx framevideo snapshot . --frames 3`)
