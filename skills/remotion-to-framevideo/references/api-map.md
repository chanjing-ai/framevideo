...
- **`refuse + interop`** = the skill bows out and recommends the runtime adapter
  pattern from [PR #214](https://github.com/chanjing/framevideo/pull/214).
...
| `staticFile("x.png")`                  | `"assets/x.png"` — copy the file into `fv-src/assets/` next to `index.html` |
...
| `slide()`, `wipe()`, `clockWipe()`, `fade()`                                   | HF [shader-transitions](https://framevideo.chanjing.com/catalog/blocks) package presets — pick the closest |
...
| Remotion                        | FrameVideo                                                                                                       |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `<Lottie animationData={data}>` | `<div id="lottie-N">` + `<script>const anim = lottie.loadAnimation({...}); window.__fvLottie.push(anim)</script>` |
