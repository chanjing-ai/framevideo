...
Use `window.__framevideo.fitTextFontSize()`:

```js
var result = window.__framevideo.fitTextFontSize(group.text.toUpperCase(), {
  fontFamily: "Outfit",
  fontWeight: 900,
  maxWidth: 1600,
});
...
Before building caption styles from scratch, check the registry — 15 ready-to-use caption components cover the most common styles. Install with `npx framevideo add <name>` and use as a sub-composition via `data-composition-src`.

```bash
npx framevideo catalog --tag caption-style   # list all caption components
npx framevideo add caption-highlight         # install a specific one
```
