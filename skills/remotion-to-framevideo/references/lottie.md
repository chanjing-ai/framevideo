...
[Lottie adapter](https://github.com/chanjing/framevideo/blob/main/packages/core/src/runtime/adapters/lottie.ts)
that supports both `lottie-web` and `@lottiefiles/dotlottie-web`. The
adapter auto-discovers animations registered on `window.__fvLottie`
...
- `window.__fvLottie.push(anim)` is what hooks the animation into HF's
  per-frame seek
...
1. Copy `hello.json` from the Remotion project into `fv-src/assets/`.
...
onto `window.__fvLottie` and the adapter will seek all of them in sync:

```js
window.__fvLottie.push(anim1);
window.__fvLottie.push(anim2);
window.__fvLottie.push(anim3);
```
...
Per the [Lottie adapter](https://github.com/chanjing/framevideo/blob/main/packages/core/src/runtime/adapters/lottie.ts)
