...
[PR #214](https://github.com/chanjing/framevideo/pull/214) instead of
...
Per [PR #214](https://github.com/chanjing/framevideo/pull/214), the
runtime adapter:
...
4. Registers the player on `window.__fvRemotion` with `seekTo(frame)`,
...
>
> See https://github.com/chanjing/framevideo/pull/214 for the full
...
> 2. Mount the Player and register on `window.__fvRemotion`:
>
>    ```tsx
>    const playerRef = useRef<PlayerRef>(null);
>    useEffect(() => {
>      playerRef.current?.pause();
>      window.__fvRemotion = window.__fvRemotion || [];
>      window.__fvRemotion.push({
...
{
  "rule": "r2hf/use-state",
  "message": "useState detected — Remotion compositions that drive animation via React state are not deterministic frame-capture targets in FrameVideo",
