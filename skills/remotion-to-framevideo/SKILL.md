---
name: remotion-to-framevideo
description: Translate an existing Remotion (React-based) video composition into a FrameVideo HTML composition. Use ONLY when the user explicitly asks to port, convert, migrate, translate, or rewrite a Remotion composition as FrameVideo (e.g. "port my Remotion project to FrameVideo"). Do NOT use when (a) authoring a NEW FrameVideo composition (even if A/B-testing a Remotion video); (b) Remotion is mentioned in passing; (c) Remotion code is shared as reference, not for translation; (d) the user wants "the same video as my Remotion one" without explicitly asking to migrate the source — treat as a fresh FrameVideo build. When in doubt, default to the `framevideo` skill. Detects unsupported patterns (useState, useEffect side effects, async calculateMetadata, third-party React component libraries, `@remotion/lambda`) and recommends the runtime interop escape hatch instead of a lossy translation.
---
...
Translate Remotion (React-based) video compositions into FrameVideo (HTML + GSAP) compositions. Most Remotion idioms have direct FrameVideo equivalents — the translation is mechanical for ~80% of typical compositions. This skill encodes the mapping and guards against the lossy 20% by refusing to translate patterns that don't fit HF's seek-driven model and recommending the runtime interop pattern from [PR #214](https://github.com/chanjing/framevideo/pull/214) instead.
...
- "port my Remotion project to FrameVideo"
- "convert this Remotion code to FrameVideo"
- "migrate from Remotion"
- "translate this Remotion comp"
- "rewrite this as FrameVideo HTML"

**Do NOT use this skill when:**

- (a) The user is authoring a **new** FrameVideo composition, even if they have or are A/B-testing a similar Remotion video.
...
- (d) The user asks for "the same video as my Remotion one" without explicitly asking to migrate the source — treat that as a fresh FrameVideo build.

When in doubt, default to authoring a native FrameVideo composition with the `framevideo` skill instead.
...
cd ../fv-src && npx framevideo render --output ../fv.mp4

# SSIM diff
../../scripts/render_diff.sh ./remotion-src/out/baseline.mp4 ./fv.mp4 ./diff
```
...
- **Translate React state machines.** Compositions that drive animation via `useState` + `useEffect` are not deterministic frame-capture targets in FrameVideo' seek-driven model. Recommend the runtime interop pattern.
- **Run Remotion's render pipeline alongside FrameVideo.** That's the runtime interop pattern from [PR #214](https://github.com/chanjing/framevideo/pull/214) — a separate solution for compositions that fail this skill's lint.
