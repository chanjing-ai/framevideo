#!/usr/bin/env node
// contrast-report.mjs — FrameVideo contrast audit
//
...
//
// Usage:
//   node skills/framevideo/scripts/contrast-report.mjs <composition-dir> \
...
import { framevideoPackageSpec, importPackagesOrBootstrap } from "./package-loader.mjs";

// Use the producer's file server — it auto-injects the FrameVideo runtime
// and render-seek bridge, so raw authoring HTML works without a build step.
const packages = await importPackagesOrBootstrap(["@framevideo/producer", "sharp"], {
  npmPackages: [framevideoPackageSpec("@framevideo/producer"), "sharp@0.34.5"],
});
...
const OUT_DIR = resolve(args.out ?? ".framevideo/contrast");
