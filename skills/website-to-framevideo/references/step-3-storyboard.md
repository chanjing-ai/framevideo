...
Pick from the inventory in [capabilities.md](capabilities.md) and implementation patterns in [techniques.md](../../framevideo/references/techniques.md). Examples of composable beats:
...
- **[techniques.md](../../framevideo/references/techniques.md)** — 13 primitive animation techniques with code patterns. Pick for beats, these are starting points to adapt, not templates to copy.
- **[text-effects.md](../../framevideo/references/text-effects.md)** — 24 named text animation effects from the separate `pixel-point/animate-text` skill. The reference page tells you how to load the upstream skill; the IDs are listed inline. Assign a specific effect ID to every headline, label, and copy element in every beat — not generic "fades in" descriptions.
...
**Global guardrails** — read [video-composition.md](../../framevideo/references/video-composition.md) first. It defines the medium rules: density, color presence, scale, frame composition, and how design.md is brand truth not layout spec. Then apply these capture-specific additions:
...
npx framevideo catalog --type block 2>/dev/null | head -40
```

There might be VFX blocks available (vfx-liquid-glass, vfx-iphone-device, vfx-shatter, vfx-portal, etc.), use them for hero treatments instead of basic perspective tilt. You need to install any you want with `npx framevideo add <name>`. Don't use too many shaders — maximum 2 per video unless user wants differently.
...
The `drawElementImage` Chrome API captures any live HTML/CSS as a GPU-accelerated texture at 60fps. This is FrameVideo' highest-impact capability — it lets you render captured product screenshots or UI through:
...
When planning beats, decide which ones deserve an HTML-in-Canvas treatment vs. a standard GSAP animation. If you want it, name it in the storyboard — Step 5 will read [`../../framevideo/references/html-in-canvas-patterns.md`](../../framevideo/references/html-in-canvas-patterns.md) for implementation. You don't need to specify the API details here.
...
```bash
find "$HOME" -path '*/website-to-framevideo/assets/sfx/manifest.json' -maxdepth 10 2>/dev/null | head -1
```
...
**Volume when SFX overlaps narration:** FrameVideo has no automatic audio ducking. If an SFX plays under spoken narration, set its volume to 0.2–0.3 max, not 0.5+. Specify this in the storyboard entry so Step 5 wires it correctly.
...
If the storyboard calls for a MacBook or iPhone mockup, use the pre-built `vfx-iphone-device` registry block — it has both **iPhone 15 Pro Max AND MacBook Pro** GLTF models with live HTML-in-Canvas screens, camera choreography, and glass lens morphing. Install with `npx framevideo add vfx-iphone-device`.
...
Every text element in this beat must name a specific effect from the catalog. The reference page is at [`../../framevideo/references/text-effects.md`](../../framevideo/references/text-effects.md) (or locate it with `find "$HOME" -path '*/framevideo/references/text-effects.md' -maxdepth 10 2>/dev/null | head -1`). It lists 24 effect IDs (from the separate `pixel-point/animate-text` skill); pick what fits the brand and this beat's mood — don't default to the same effect every beat.
...
The two beats below are from the real Claude Design × FrameVideo production video. They show the expected level of specificity — exact timing, exact GSAP values, exact animation sequences.

**Why only 2 beats are shown:** Earlier versions of this reference showed all 10 beats, and agents pattern-matched from them regardless of the brand being captured. Moodboard layouts, capabilities grids, and orbital letter closers started appearing in every video. The concepts in those beats are specific to FrameVideo as a product — they should not appear in a video about a fintech tool or a wellness app. Only two beats are shown here to demonstrate the format level, not to suggest these specific techniques.
...
- 1.0s: Title "Claude Design × FrameVideo" fades up above the line (opacity: 0→1, y: 14→0, 0.7s, power3.out). Ampersand in italic accent color.
- 1.3s: Date subtitle appears below the line (0.6s, power2.out). Monospace font, 0.32em letter-spacing, uppercase.
- 2.2s: Bottom credit line fades in ("This entire video was made with FrameVideo in Claude Design")
