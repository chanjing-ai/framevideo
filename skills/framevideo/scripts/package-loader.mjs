...
const BOOTSTRAP_ENV = "FRAMEVIDEO_SKILL_DEPS_BOOTSTRAPPED";
const BOOTSTRAP_CONFIRM_ENV = "FRAMEVIDEO_SKILL_BOOTSTRAP_DEPS";
const NODE_MODULES_ENV = "FRAMEVIDEO_SKILL_NODE_MODULES";
...
export function framevideoPackageSpec(packageName) {
  const version = readBundledFrameVideoVersion();
  if (!version) {
    throw new Error(
      [
        `Could not determine the bundled FrameVideo version for ${packageName}.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__framevideo_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledFrameVideoVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "framevideo" || manifest.name === "@framevideo/cli") {
...
async function confirmBootstrap(packageSpecs) {
  ...
  try {
    const answer = await rl.question(
      [
        "FrameVideo helper package(s) are missing.",
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "framevideo-skill-deps-"));
