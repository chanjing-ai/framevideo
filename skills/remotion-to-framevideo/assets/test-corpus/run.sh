...
#   - HF CLI built at packages/cli/dist/cli.js (run `bun run --filter @framevideo/cli build`
...
FV_CLI="$REPO_ROOT/packages/cli/dist/cli.js"
...
require_render_tier_tools() {
  if [[ ! -f "$FV_CLI" ]]; then
    echo "error: HF CLI not built at $FV_CLI" >&2
    echo "       Run 'bun run --filter @framevideo/cli build' in $REPO_ROOT" >&2
    return 2
  fi
...
run_render_tier() {
  ...
  if ! (cd "$fixture_dir" && \
        node "$FV_CLI" render fv-src/ --output fv.mp4 --quiet >/dev/null 2>&1); then
    echo "    ✗ HF render failed"
    write_result "$fixture_name" "fail" stage "fv-render"
    return 0
  fi

  if R2FV_SSIM_THRESHOLD="$threshold" "$DIFF" \
...
echo "remotion-to-framevideo corpus run"
echo "=================================="
